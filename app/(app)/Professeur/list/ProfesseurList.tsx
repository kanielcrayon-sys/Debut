...
export default function ProfesseurList() {
  ...
  const [filteredProfsSearch, setFilteredProfsSearch] = useState<Professeur[]>([]);
  ...
  useEffect(() => {
    const performSearch = async () => {
      try {
        if (searchTerm.trim() === "") {
          setFilteredProfsSearch(displayedProfs);
          return;
        }

        const q = query(
          collection(db, "professeurs"),
          where("statut_enseignant", "==", showTrash ? "abandonné" : "actif")
        );
        ...
        const filtered = results.filter((prof) =>
          `${prof.identite.prenom_individu} ${prof.identite.nom_individu}`
            .toLowerCase()
            .includes(searchTerm.toLowerCase())
        );
        
        setFilteredProfsSearch(filtered);
      } catch (error) {
    ...
    <div className="w-full p-6">
      ...
      <div className="overflow-x-auto shadow-md rounded-lg mb-8">
        <table className="w-full border-collapse bg-white dark:bg-gray-800">
          ...
          <tbody>
            {profsWithStats.length > 0 ? (
              profsWithStats.map((professeur) => (
                <tr
                  key={professeur.id}
