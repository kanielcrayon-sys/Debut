...
export default function MatiereList() {
  ...
  const [filteredMatieresSearch, setFilteredMatieresSearch] = useState<Matiere[]>([]);
  ...
  useEffect(() => {
    const performSearch = async () => {
      try {
        if (searchTerm.trim() === "") {
          setFilteredMatieresSearch(displayedMatieres);
          return;
        }

        const q = query(
          collection(db, "matieres"),
          where("statut_matiere", "==", showTrash ? "abandonné" : "actif")
        );
        ...
        const filtered = results.filter((matiere) =>
          matiere.libelle_matiere.toLowerCase().includes(searchTerm.toLowerCase())
        );
        
        setFilteredMatieresSearch(filtered);
      } catch (error) {
    ...
    performSearch();
  }, [searchTerm, displayedMatieres, showTrash]);
//userRoleGuard
    if (loadingRole) {
    return (
    ...
    <div className="w-full p-6">
      ...
      <div className="overflow-x-auto shadow-md rounded-lg mb-8">
        <table className="w-full border-collapse bg-white dark:bg-gray-800">
          ...
          <tbody>
            {filteredMatieresSearch.length > 0 ? (
              filteredMatieresSearch.map((matiere) => (
                <tr
                  key={matiere.id}
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center text-sm text-gray-900 dark:text-white">
                    ...
                    <span className={`px-3 py-1 rounded-full ${
                      matiere.qualificatif === "Fondamentale" 
        ...
        <DialogContent className="dark:bg-gray-800 mt-4">
          {selectedMatiereForInfo && (
            <div className="flex flex-col gap-3">
              ...
              <div>
                <span className="font-semibold text-gray-900 dark:text-white">Qualificatif:</span>
                <span className="ml-2 text-gray-700 dark:text-gray-300">
                  {selectedMatiereForInfo?.qualificatif || "Fondamentale"}
                </span>
              </div>
              <div>
